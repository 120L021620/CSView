...
1.首先，当A的构造方法实例化完成后会被加入到三级缓存（singletonFactories）中，以便在解析依赖的过程中使用。
2.此时，进行B的构造方法实例化。在解析到B依赖A时，会检查二级缓存和一级缓存中是否存在依赖的A，发现暂时没有提前创建好的A实例。
3.接着，Spring会检查三级缓存（singletonFactories）中是否有A实例相关的工厂。这里可以找到创建A的工厂，因此Spring会用这个工厂提前暴露出一个还未填充属性和初始化的A实例。这个提前暴露的A实例会被放入二级缓存（earlySingletonObjects）。
4.B的构造方法实例化完成后，会将A实例从二级缓存（earlySingletonObjects）注入到B中，此时A和B之间的循环依赖被解决。
5.接下来，进行Bean属性填充和调用初始化方法，最终将完全初始化后的A和B实例放入一级缓存（singletonObjects）。
需要注意的是，三级缓存仅能解决基于setter注入和字段注入的循环依赖，无法解决构造器注入的循环依赖问题。同时，三级缓存也不适用于原型作用域Bean。
